...
> **β10.0-α 実装メモ**: 25×25 サイズ追加とタッチ環境対応のため、`src/game/store.ts` に
> `viewport: { scale, panX, panY }` を導入し、Pixi.js v8 の `boardRoot` Container に
> `scale + position` を適用する **ズーム+パン UI** を実装した。
> - 1 本指 (描画) と 2 本指 (ピンチ + pan) を 'idle' / 'drawing' / 'gesture' 状態で排他制御
> - マウスホイール zoom (anchor 中心保持)、Mac トラックパッドは ctrlKey 分岐 (ピンチ/swipe)
> - HUD に + / − / ⤢ ボタン (a11y フォールバック)
> - viewport は localStorage 非永続 (端末固有 / Gemini Pro 指摘)
> - pan ±10000 緩いセーフティクランプ (浮動小数精度落ち防止)
> - 入力座標変換: `pickCell(clientX, clientY)` で `(canvas - pan) / scale` の逆変換適用

## 7.1 ビューポートとは

ピクセルズの **ビューポート** は、画面に表示される領域。当初 (MVP) は ≤15×15 で全体表示前提だったが、
β6.0-α で 25×25 を追加 + β10.0-α でズーム+パン UI を実装し、大盤面でのタッチ操作も実用可能になった。

オートフィット (基本表示) + ユーザー操作によるズーム+パン (拡大時) のハイブリッド構成。
