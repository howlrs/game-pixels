...
- **復帰時にゲームを自動再開しない**理由: ① ノノグラムでは物理状態の事故は起きないが、復帰タイミングでタイマーが急に動き出すと混乱する。② iOS の AudioContext unlock がユーザージェスチャー必須のため、画面タップを必須化することで音とタイマーを同時に確実に復帰させる。
...
- ボス戦に入ると BGM 切替。
- ノノグラム (本作) はリアルタイム性が低いため、BGM テンポ変化やクリア時のフェード等は MVP には入れない。BGM は単純なループのみ。
- ポーズ時は BGM を suspend する (フェードはしない)。

## 12.5 SE カタログ (MVP)

ピクセルズ用に絞り込み (旧プラットフォーマー用 SE 一覧は Round 5 で全置換):

| イベント | SE | 備考 |
|---|---|---|
| セルを塗 (Fill) | fill_pop | 「ぽちっ」軽い音、連打で疲れない |
| × をつける (MarkX) | mark_cross | 軽い「コツン」音、Fill と区別できる音色 |
| セルを消す (Erase) | erase_swipe | 「サッ」と消える音 |
| ヒント数字を取り消し線トグル | clue_check | 「カチッ」軽い音 |
| パズルクリア (完成) | puzzle_clear | 短いファンファーレ |
| ベストタイム更新 | best_time | 上記より更に短い「キラッ」音 |
| 盤面リセット (確認後) | board_reset | 「ザザッ」全消去音 |
| エラー / 操作不能 | error | 「ブブッ」短い禁止音 (タップ無効箇所など) |

> 旧仕様の「ジャンプ / 敵踏み / コイン / 死亡 / ファイア弾 / ゲームオーバー」等は **すべて削除** (ジャンル不一致)。
